...
public:
	...
	void swap_kth(int k)
	{
		...
		Node *backward_pre = backward->prev;
		Node *forward_next = forward->next;
		...
		{
			link(forward->prev, backward);
			link(forward, backward->next);
		}

		if (forward_next == backward)
		{
			link(backward, forward);
		}
		else
		{
			link(backward, forward_next);
			link(backward_pre, forward);
		}
