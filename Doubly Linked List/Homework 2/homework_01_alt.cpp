...
private:
	...
	void Internal_find_the_middle(Node *curr, int length, int &middle_idx, int &middle)
	{
		if (curr == nullptr)
		{
		...
		length++;
		Internal_find_the_middle(curr->next, length, middle_idx, middle);
		if (length == middle_idx)
			middle = curr->data;
	}
...
public:
	...
	int find_the_middle()
	{
		...
		int middle_idx = -1;
		int middle;
		Internal_find_the_middle(head, 0, middle_idx, middle);
		return middle;
	}
};
...
void test1()
{
	...
	int expected = 3;
	int result = list.find_the_middle();
	if (expected != result)
	{
		cout << "no match:\nExpected: " << expected << "\nResult  : " << result << "\n";
		assert(false);
